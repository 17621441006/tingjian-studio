# 庭间 v28 · 橘黄阳台与四种客厅地面对照

- 修正第十套阳台理解偏差：恢复新中式第二套原阳台的橘黄暖木顶、墙、地、坐榻与茶几，重绘阳台与客厅2中可见阳台，室内深木墙和棕皮沙发保留。
- 客厅新增三张独立地面对照图：柔棕胡桃木直铺（首次选第十套默认）、烟橡人字拼、暖灰石板；原浅橡木仍可选。入口合并在“家具与材料 → 地面”，首页与第三步均可点击，选整图自动清除品牌叠合。
- 选择沿用variant状态，保留到全屋确认与导出；餐厅确认沿用含餐区的客厅视角。客厅2仍为固定基准视角，地板选配不联动反向图；三维仍为原布局近似，未宣称本次生成扫描模型。
- 五张新主图均1536×1024，新增/替换含缩略图共10份、约1.8MB；原浅木图和其他方案不降分辨率。素材在dist/designs-v26，部署沿用固定GitHub提交素材来源。v26清单更新4条替换项；v28保留新素材哈希、被替换记录、完整提示词。
- 检查包含实际图片解码/SHA、地面切换与品牌互斥、房间范围、确认/导出快照与恢复原图。构建与现有媒体检查沿用；未进行真实手机或浏览器视觉验收。归档与LFS不变。

---

# 庭间 v27 · 第十套改为暮木茶境

- 用户否定第十套虚构凸墙，要求第1套木墙板＋适配木地板＋第2套阳台。用暮色原图重绘客厅、餐厅、反向客厅2、阳台，齐平连续墙板和原空间关系；不使用旧第十套图作为格局依据。
- 第10套更名“暮木茶境”，稳定内部ID milan保持已有选择兼容。深胡桃木饰面配较浅中性烟熏橡木地板，棕皮沙发、浅石茶几；阳台采用第二套的木廊顶/侧墙、尽端软垫坐榻、小茶几和植物。
- 替换4张1536×1024主图及4缩略图，更新3房间的近似地板遮罩。v26/assets.json代表当前素材；被替换素材哈希留在v27/replaced-assets.json，原版本仍由Git历史保留。其余方案与第10套其他房间图未改。
- 概念3D对应改为棕皮沙发、中性木地板和阳台坐榻；模型仍为布局近似，不宣称由新图扫描。双视角仍是原搭配，后续单品改动暂不联动客厅2。
- 通过构建、图片实际解码与尺寸/SHA检查、十套八房间及双视角UI回归、真实Three几何检查。4张新图逐张检查，未进行手机/浏览器视觉验收。维持现有共享范围、原VR分辨率、LFS与归档。

---

# 庭间 v26 · 十套完整方案与客厅双视角

- 保留原七套，新增08橡影 EDITION、09烟木 EDITION、10胡桃艺廊。两套Edition从浅橡/烟熏木地板出发整体设计，第三套意式现代用胡桃木、酒红与浅灰石。每套8房间，次卧、卫浴、阳台分别重新构思。
- 新增24房间图+10套客厅反向图，共34张原生1536×1024图片及34张480×320缩略图。生成采用内置image_gen，完整prompt与来源在verification/v26/*-prompts.json，字节/尺寸在assets.json。原图片/模型/全景未修改。
- 客厅2作为同一房间的附加视角，首页、布局、选材、全屋确认均可切换和放大；不增加第9个房间确认项。明确显示是原方案反向图，不联动后续单品、地板或装饰修改；编辑后回到当前正向图。导出册附带客厅2。
- 新方案沿用8室概念三维，配置独立配色、木地板、次卧/阳台布置分支；它是布局近似模型，不是由新照片扫描建模。新增24份保守地板遮罩，品牌换地板仍是近似叠合。新方案目前支持整体图、品牌地面与选购清单，没有宣称具备旧7套的逐物删除补全。
- 新素材位于dist/designs-v26，本地克隆自包含；Sites输出仍受256MiB限制，新增素材由固定GitHub素材提交提供，prepare-hosting将来源写入legacy.html，现有v25来源不变。首次打开新方案/客厅2需联网。归档分支与LFS未修改，仓库仍低于1GB。
- 检查覆盖十套选择、每套8室确认、双视角/大图/恢复、失败与过期请求、原媒体SHA、34张新主图与缩略图实际解码、Three.js几何。图片逐张审阅；未进行真实浏览器/手机视觉验收。

---

# 庭间 v25 · 首页排版、逐室删除与装饰切换

- 首页房间导航及材质故事移到主图正下方，形成房间缩略图与说明；右侧工具栏独立滚动，窄屏恢复普通流。
- 七套方案、八个房间共352个图像对象控制。补上铜色客厅茶几、餐桌椅、展示架；删除/加回、客餐厅联动、其他房间独立、进入全屋确认保留选择。原布局支持编辑；其他布局/变体需要先回原方案。固定门窗、厨柜和家政柜保留。
- 新增56张补全背景与11张装饰视图，原图不覆盖，原装饰/新装饰可切换。家具为图像区域遮罩，补全与边缘近似，并非三维扫描分割；Three.js已有对应对象同步显隐，新增装饰为独立几何。原VR仍是固定基准全景。
- 原始图片、8张全景和模型不降分辨率。新WebP均1536×1024，67份共14,325,198字节。完整dist可本地运行；Sites输出因256MiB限额排除新增素材目录，以固定GitHub提交c7167efd1f03ebbe660b2abdb4256a8c17a6c301提供新增素材，首次编辑需联网。dist默认使用本地素材。
- 检查覆盖67份图片实际解码、房间/删除/还原/装饰状态、Three.js对象、确认页持久状态及原媒体哈希。真实canvas合成样例已检查；没有进行真实浏览器或手机视觉验收。归档分支及LFS资产未修改。

---

# 庭间 v24 · 恢复暮色实景贴图，墙体与家具分开处理

- 用户截图对应 v21 暮色全景贴图，明确要求恢复这种质感；v22整体移除投影过度简化，现恢复8张原生全景的家具/地面投影，专门跳过直墙与窗下墙，使用平直实体及独立木/石/灰泥材质。已有品牌地面优先于全景纹理。
- 家具仍为各自独立 Group/objectId，保留细节模型全部顶点、软包、被褥、桌椅、柜体与饰品几何，不进行全屋合并或降面。照片里的附加细节仍是二维纹理，并非扫描后分割的真实对象；未声称消除家具投影错位。
- 按需加载全景，保留稳定的静止/拖动渲染路径；按钮为“暮色实景贴图”，载入状态及固定基准纹理限制明确展示。其余风格、v23音乐、布局及照片换地板保持。
- 原图/全景字节与分辨率保持，未新增大媒体。自动检查覆盖完整几何/对象身份不变、8室投影、墙体排除、品牌地面优先、原媒体、UI与构建；当前静态站未完成浏览器/手机视觉验收。
