...
assert.equal(TRACKS.length,8);let bytes=0;const durations=[];
for(const track of TRACKS){const b=await fs.readFile('dist'+track.path);assert.equal(createHash('sha256').update(b).digest('hex'),track.sha256);bytes+=b.length;const info=JSON.parse(execFileSync('ffprobe',['-v','quiet','-show_entries','format=duration:stream=channels,sample_rate','-of','json','dist'+track.path],{encoding:'utf8'}));assert(Number(info.format.duration)>30);assert.equal(info.streams[0].channels,2);durations.push(Number(info.format.duration));}
