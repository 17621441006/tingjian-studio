...
await fs.cp('dist','build',{recursive:true,filter:p=>!p.includes('.openai')&&!p.includes('scene-edits-v25')&&!p.includes('designs-v26')});
...
const {origin}=JSON.parse(await fs.readFile('verification/v26/media-origin.json','utf8'));
const html=await fs.readFile('build/tour/legacy.html','utf8');
await fs.writeFile('build/tour/legacy.html',html.replace('</head>', '<script>globalThis.__TINGJIAN_V26_ORIGIN__='+JSON.stringify(origin).replaceAll('<','\\u003c')+';</script></head>'));
