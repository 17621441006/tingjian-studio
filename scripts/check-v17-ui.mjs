...
assert.equal($(floorHome+'[data-floor-category="wood"]').getAttribute('aria-pressed'),'true');
assert.equal($('[data-home-floor-catalog]').querySelectorAll('[data-floor-product]').length,5);
...
await click(floorDetails+'[data-floor-category="tile"]',5);assert.equal($('[data-details-floor-catalog]').querySelectorAll('[data-floor-product]').length,5);
...
await click(floorDetails+'[data-floor-category="wood"]',4);for(const b of $('[data-details-floor-catalog]').querySelectorAll('[data-floor-product]'))assert(b.disabled);
await click('[data-details-room="second"]',15);await click(floorDetails+'[data-floor-category="wood"]',5);await click(floorDetails+'[data-floor-product="kahrs-limestone"]',15);
