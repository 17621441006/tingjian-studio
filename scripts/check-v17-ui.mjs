...
await click('[data-journey-step="style"]',8);await click('[data-home-design="copper"]',20);await click('[data-home-room="living"]',15);
await click('[data-home-object-controls] [data-scene-object="table"]',20);assert(context.__homeGallery.getState().removed.includes('table'));
await click('[data-home-object-controls] [data-partition-style="custom"]',20);assert.equal(context.__homeGallery.getState().partitionStyle,'custom');
await click('[data-home-room="master"]',15);assert(!context.__homeGallery.getState().removed.includes('table'));await click('[data-home-object-controls] [data-scene-object="bed"]',20);assert(context.__homeGallery.getState().removed.includes('bed'));
await click('[data-home-room="living"]',15);assert(context.__homeGallery.getState().removed.includes('table'));assert.equal(context.__homeGallery.getState().partitionStyle,'custom');
await click('[data-confirm-style]',25);await click('[data-layout-next]',15);await click('[data-details-room="master"]',20);assert(context.__journey().current.removed.includes('bed'));await click('[data-details-room="living"]',20);assert(context.__journey().current.removed.includes('table'));assert.equal(context.__journey().current.partitionStyle,'custom');
...
for(const design of ['edition-oak','edition-smoke','milan']){
 ...
 await click('[data-home-large]',3);assert.equal($('[data-home-dialog-image]').src,livingReverseAsset(design));await click('[data-home-close]',3);
 await click('[data-home-camera] [data-living-camera="front"]',10);assert.equal($('[data-home-image]').src,base);
 await click('[data-confirm-style]',20);await click('[data-layout-next]',10);await click('[data-confirm-all]',35);await click('[data-details-next]',35);
 assert.equal(context.__journey().whole.frames.length,8);assert.equal(context.__journey().whole.design,design);
 await click('[data-model-room="living"]',5);await click('[data-model-camera] [data-living-camera="reverse"]',10);assert.equal($('[data-current-model-photo]').src,livingReverseAsset(design));
}
await click('[data-journey-step="style"]',8);await click('[data-home-design="graphite"]',20);await click('[data-home-room="living"]',15);
