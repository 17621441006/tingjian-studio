...
export function resolveScene(s={}){
 ...
 return {...base,room,mode:design==='dusk'&&s.mode!=='palette'?'pieces':'palette',variant:resolveVariantState(design,room,s.variant).variant,pieces,window};
}
...
export function sceneNote(value){
 const s=resolveScene(value);if(s.mode==='palette')return '正在保留这组整体搭配。若想独立换单品，请在第三步选择原布局的单品搭配。';
...
export function sceneItems(value){
...
export function assembleHome(design,saved,seed={}){
 const list=ROOMS.map(r=>{const scene=resolveScene(saved.get(design+':'+r.id)||{...seed,design,room:r.id,layout:'original',light:'daywarm',window:'original'});return {id:r.id,name:r.name,area:r.area,scene,path:sceneAsset(scene),thumb:sceneAsset(scene,true),label:sceneLabel(scene),items:sceneItems(scene),changed:saved.has(design+':'+r.id)};});
 const living=list.find(f=>f.id==='living'),dining=list.find(f=>f.id==='dining');
 if(living.changed){Object.assign(dining,{path:living.path,thumb:living.thumb,label:'与客厅共用已选视角',items:[['客餐厅','共用当前客餐厅画面，餐区位于画面后方']],shared:true,changed:true});}
 return list;
}
