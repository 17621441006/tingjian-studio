...
export function pieceAvailable(room,selection){const s=normalizePieces(selection);return room!=='living'||s.floor==='stone'||['cognac','blend','slim'].includes(s.sofa)&&s.table==='glass';}
export function pieceAvailabilityNote(room,selection){return pieceAvailable(room,selection)?'':'木地面目前配干邑、皮框细织或烟褐纤细框沙发＋烟玻璃茶几。先改回对应单品，再试木地面；不会自动替你改色或换款。';}
