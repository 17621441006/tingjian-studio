...
export function initHomeGallery(root){
 ...
 const assetFor=(view,selection=pieces,thumb=false)=>isPieceView(view)?pieceAsset(view.room,selection,thumb):variantAsset(view.design,view.room,view.variant,thumb);
 ...
 function renderRooms(){
  ...
  for(const [i,room]of ROOMS.entries()){
   ...
   number.textContent=String(i+1).padStart(2,'0')+(displayed.design==='dusk'&&PIECE_ROOMS.includes(room.id)?' · 单品选配':variantsFor(displayed.design).length&&VARIANT_ROOMS.includes(room.id)?' · 搭配对照':'');
 ...
 function paint(){
  ...
  const label=individual?content.label:applied?variant.name:variant?'本空间沿用原搭配':'原搭配',path=assetFor(displayed);
  ...
  const dl=$('[data-home-materials]');dl.replaceChildren();for(const [name,note] of content.materials){const div=document.createElement('div'),dt=document.createElement('dt'),dd=document.createElement('dd');dt.textContent=name;dd.textContent=note;div.append(dt,dd);dl.append(div);}
  ...
  document.dispatchEvent(new CustomEvent('tingjian:home-view',{detail:{...displayed,pieces:{...pieces},path,thumb:assetFor(displayed,pieces,true)}}));
 }
 async function select(design,room,variant=choices[design]||'original',mode=modes[design]||'palette',selection=intended.pieces){
  const next={...resolveVariantState(design,room,variant),mode:design==='dusk'&&mode==='pieces'?'pieces':'palette'},snapshot=normalizePieces(selection),id=++requestId;
  ...
  try{
   ...
   displayed=next;pieces=snapshot;choices[next.design]=next.variant;modes[next.design]=next.mode;paint();return true;
  }catch(error){
   if(id!==requestId||root.dataset.surface!=='home')return false;
   intended={view:{...displayed},pieces:{...pieces}};
   const status=$('[data-home-load-status]');status.textContent=error.message+'。保留当前画面。';const retry=document.createElement('button');retry.type='button';retry.textContent='重新载入';retry.addEventListener('click',()=>select(next.design,next.room,next.variant,next.mode,snapshot));status.append(' ',retry);
   return false;
 ...
 $('[data-home-large]').addEventListener('click',event=>{
  ...
  $('[data-home-dialog-view]').classList.remove('is-native');$('[data-home-native]').setAttribute('aria-pressed','false');openImageDialog(dialog,window);
 });
 ...
 return {setRoomHandler:fn=>{roomHandler=fn;},open:()=>select(displayed.design,displayed.room,displayed.variant,displayed.mode),openView:select,getState:()=>({...displayed,pieces:{...pieces},path:assetFor(displayed),thumb:assetFor(displayed,pieces,true)})};
}
