...
export function createMusicPlayer(audio,{onChange=()=>{},save=()=>{},volume=.18}={}){
 ...
 function setTheme(id){id=({'edition-oak':'amber','edition-smoke':'dusk',milan:'dusk'})[id]||id;if(theme===id||!TRACKS.some(t=>t.id===id))return;theme=id;choose(TRACKS.findIndex(t=>t.id===id));}
...
export function mountMusic(root){
 ...
 for(const [i,t] of TRACKS.entries()){const o=document.createElement('option');o.value=i;o.textContent=t.collection+' · '+t.title+' · '+t.style.split(' / ')[0];select.append(o);}
 let prefs={};try{prefs=JSON.parse(localStorage.getItem('tingjian-music')||'{}');}catch{}
 const player=createMusicPlayer(audio,{volume:Number.isFinite(prefs.volume)?(prefs.volume===.3?.18:Math.min(1,Math.max(0,prefs.volume))):.18,onChange:s=>{select.value=s.index;button.textContent=s.enabled?'关闭音乐':'开启音乐';button.setAttribute('aria-pressed',String(s.enabled));root.classList.toggle('is-playing',s.enabled&&!s.blocked);root.querySelector('[data-disc-art]').src='/music/art/'+(s.track.art||s.track.id)+'.svg';root.querySelector('[data-disc-symbol]').textContent=s.enabled&&!s.blocked?'Ⅱ':'▶';root.querySelector('[data-disc-toggle]').setAttribute('aria-label',s.enabled?'暂停背景音乐':'播放背景音乐');root.querySelector('[data-music-title]').textContent=s.track.title;root.querySelector('[data-music-state]').textContent=s.blocked?'轻触页面即可播放':s.enabled?'正在播放 · 顺序循环':'音乐已关闭';},save:s=>{try{localStorage.setItem('tingjian-music',JSON.stringify(s));}catch{}}});
