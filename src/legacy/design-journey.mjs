...
export function initDesignJourney(root,gallery){
 ...
 function renderWhole(){
  ...
  const active=result.frames.find(f=>f.id===wholeRoom)||result.frames[0];const whole_cameraReady=setFloorPhoto($('[data-whole-room-image]'),active.path,active.scene,$('[data-whole-room-save]'));renderLivingViews($('[data-whole-camera]'),$('[data-whole-room-image]'),active.scene,whole_cameraReady,()=>{setFloorPhoto($('[data-whole-room-image]'),active.path,active.scene,$('[data-whole-room-save]'));},[$('[data-whole-room-save]')]);$('[data-whole-room-image]').alt=active.name+' · '+active.label;$('[data-whole-room-title]').textContent=active.name+' · '+active.label;
  ...
  const model_cameraReady=setFloorPhoto($('[data-current-model-photo]'),active.path,active.scene);renderLivingViews($('[data-model-camera]'),$('[data-current-model-photo]'),active.scene,model_cameraReady,()=>{setFloorPhoto($('[data-current-model-photo]'),active.path,active.scene);},[]);$('[data-current-model-photo]').alt=active.name+' · '+active.label;$('[data-current-model-room-title]').textContent=active.name;$('[data-current-model-room-items]').textContent=active.label;
 ...
 $('[data-current-model-room-large]').addEventListener('click',()=>{const f=snapshot()?.frames.find(f=>f.id===wholeRoom);if(f)large(f.path,HOMES[confirmed].name+' · '+f.name,f.scene,$('[data-current-model-photo]'));});
